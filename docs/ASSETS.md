...
극장 체인 / 상영 포맷 로고는 **폴더의 파일명이 단일 진실의 원천**이에요. 빌드 타임 generator가 폴더를 스캔해서 `src/utils/assets.generated.ts`를 출력하고, picker / 본문 티켓 컴포넌트가 이걸 import해서 자동 동기화돼요. 자산만 드랍하면 picker에 자동 등장 — 코드 수정 불필요.
